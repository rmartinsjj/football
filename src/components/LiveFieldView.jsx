  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white p-3 border-b border-gray-100">
        <h4 className="text-gray-900 text-xs font-medium mb-2 text-center">Classificação Atual</h4>
        <div className="grid grid-cols-2 gap-2">
          {standings.slice(0, 4).map((team, index) => (
            <div key={team.team} className="flex items-center justify-between bg-gray-100 rounded-lg p-2">
              <div className="flex items-center space-x-2">
                <span className="text-gray-900 text-xs font-bold">{index + 1}º</span>
                <div className={`w-3 h-3 ${TEAM_COLORS[team.team].bg} rounded-full`}></div>
                <span className="text-gray-900 text-xs truncate">{team.team}</span>
              </div>
              <span className="text-gray-900 text-xs font-bold">{team.points}pts</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-gray-900 p-3">
        ...
        {/* Controles do Jogo - Minimalistas */}
        <div className="flex justify-center space-x-1 mb-3">
          {activeMatch !== currentMatch.id ? (
            <button
              onClick={() => startMatchTimer(currentMatch.id)}
              className="bg-green-600 hover:bg-green-700 active:bg-green-800 text-white w-8 h-8 rounded-full flex items-center justify-center transition-colors"
              title="Iniciar"
            >
              <Play size={14} />
            </button>
            ...
            <>
              {!isTimerRunning ? (
                ...
                <button
                  onClick={pauseTimer}
                  className="bg-yellow-600 hover:bg-yellow-700 active:bg-yellow-800 text-white w-8 h-8 rounded-full flex items-center justify-center transition-colors"
                  title="Pause"
                >
                  <Pause size={14} />
                </button>
              ...
              <button
                onClick={() => resetTimer()}
                className="bg-gray-600 hover:bg-gray-700 active:bg-gray-800 text-white w-8 h-8 rounded-full flex items-center justify-center transition-colors"
                title="Reset"
              >
                <RotateCcw size={14} />
              </button>
              <button
                onClick={finishMatch}
                className="bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white px-3 h-8 rounded-full flex items-center justify-center transition-colors text-xs font-medium"
                title="Finalizar"
              >
                ✓ Finalizar
              </button>
      ...
      <div className="bg-white mx-3 mb-3 rounded-xl p-3 shadow-sm border border-gray-100">
        <h3 className="text-gray-900 font-bold mb-2 text-sm text-center">
          Gols da Partida
        </h3>
        ...
        {matchEvents.filter(event => event.matchId === currentMatch.id).length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto">
            ...
            <div className="space-y-1">
              <h4 className={`text-xs font-medium text-center mb-1 ${TEAM_COLORS[currentMatch.team1]?.text || 'text-gray-900'}`}>
                {currentMatch.team1}
              </h4>
              {matchEvents
                .filter(event => event.matchId === currentMatch.id && event.teamName === currentMatch.team1)
                .map((event) => (
                  <div key={event.id} className="flex items-center justify-between bg-gray-100 rounded-lg p-2">
                    <div className="flex items-center space-x-1">
                      <span className="text-gray-900 text-xs">⚽</span>
                      <span className="text-gray-900 text-xs truncate">{event.playerName}</span>
                      <span className="text-gray-500 text-xs">{event.minute}'</span>
                    </div>
                    <button
                      onClick={() => removeGoal(event.id)}
                      className="text-red-500 hover:text-red-600 active:text-red-700 text-xs"
                    >
                      ❌
                    </button>
                  </div>
            ...
            {/* Gols do Time 2 */}
            <div className="space-y-1">
              <h4 className={`text-xs font-medium text-center mb-1 ${TEAM_COLORS[currentMatch.team2]?.text || 'text-gray-900'}`}>
                {currentMatch.team2}
              </h4>
              {matchEvents
      ...
      {/* Atualizar Placar */}
      <div className="bg-white mx-3 mb-3 rounded-xl p-3 shadow-sm border border-gray-100">
        <h3 className="text-gray-900 font-bold mb-2 text-center text-sm">Atualizar Placar</h3>
        <div className="flex items-center justify-center space-x-4">
          <div className="text-center">
            <div className="text-gray-900 font-medium mb-1 text-xs">{currentMatch.team1}</div>
            <input
              type="number"
              min="0"
              ...
              className="w-12 h-12 text-lg font-bold text-center border-2 border-gray-200 bg-white text-gray-900 rounded-lg focus:border-blue-500 focus:outline-none"
              placeholder="0"
              inputMode="numeric"
            />
          </div>
          ...
          <div className="text-center">
            <div className="text-gray-900 font-medium mb-1 text-xs">{currentMatch.team2}</div>
            <input
              type="number"
              min="0"
        ...
        <div className="space-y-1" style={{ maxHeight: '120px', overflowY: 'auto', overflowX: 'hidden' }}>
          {remainingMatches.length > 0 ? remainingMatches.slice(0, 3).map((match) => (
            <div key={match.id} className={`flex items-center justify-between rounded-lg p-2 ${
              match.played ? 'bg-green-100' : 'bg-gray-100'
            }`}>
              <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                {match.played ? (
                  ...
                  <button
                    ...
                    className="bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white px-2 py-1 rounded-lg text-xs transition-colors"
                  >
                    Iniciar
                  </button>
                )}
